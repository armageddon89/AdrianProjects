 ...
 * Author:     Adrian Dobrescu, original template - Noah Coad, http://coad.net/noah
...
/*
        ...
        int x = comport.Read(buffer, 0, 20);
    ...
    { e.Handled = KeyHandled; }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Error: Could not chose the file name: " + ex.Message);
                }
            }
        }
        ...
        public void DoWork()
        {
            try
            {
                ...
                int offset = 0;
                byte[] fileContent = File.ReadAllBytes(fileNameBox.Text);
                ...
                {
                    progress.Minimum = 0;
                    progress.Maximum = fileContent.Length;
                }));

                byte[] start = new byte[1];
                start[0] = Convert.ToByte('s');

                comport.Write(start, 0, 1);

                try
                {
                    byte[] readBuf = new byte[1];
                    comport.Read(readBuf, 0, 1);
                }
                catch (System.TimeoutException)
                {
                    this.Invoke(new Action(delegate()
                    {
                        rtfTerminal.Text += "timeout on start";
                        rtfTerminal.Update();
                    }));
                }

                byte[] len = BitConverter.GetBytes(fileContent.Length);
                comport.Write(len, 0, 4);
                ...
                catch (System.TimeoutException)
                {
                    this.Invoke(new Action(delegate()
                    {
                        rtfTerminal.Text += "timeout on sending length";
                        rtfTerminal.Update();
                    }));
                }
                ...
                while (offset < fileContent.Length)
                {
                    ...
                    {
                        length = fileContent.Length - offset;
                        break;
                    }

                    comport.Write(fileContent, offset, length);
                    ...
                    this.Invoke(new Action(delegate() 
                    {
                        ...
                        progress.Update();
                        percentage.Text = "" + 100 * offset / fileContent.Length + " %";
                        percentage.Update();
                    }));
                }
            }
